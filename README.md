...
3. 计算按压时间
 ...
  * 自动版：
    棋子坐标：由上向下逐行扫描，找出第一行符合棋子颜色的像素点，根据这些点求出的平均值就是棋子的x坐标值；
             由左向右逐列扫描，找出第一列符合棋子颜色的像素点，根据这些点求出的平均值就是棋子的y坐标值；
             
    棋盘坐标：由上向下逐行扫描，找出第一颜色发生变化的像素点，根据这些点求出的平均值就是棋盘的x坐标值；
             由左向右逐列扫描，找出第一列符合棋盘颜色的像素点，根据这些点求出的平均值就是棋盘的y坐标值；
             
    还有一些其它关键细节，具体见 wechat_jump_auto.py 文件。
