...
under_game_score_y = 300     # 截图中刚好低于分数显示区域的 Y 坐标，300 是 1920x1080 的值，2K 屏、全面屏（坚果pro2 411）请根据实际情况修改  720p时191
press_coefficient = 1.392   # 长按的时间系数，请自己根据实际情况调节 720p 2.15
...
def jump(distance):
    ...
    press_time = min(press_time, 4*1000) 
    press_time = int(press_time)
    print(distance, press_time)
	# 按压位置随机  防止触发系统识别字体
    x = random.uniform(50, 1000)
    y = random.uniform(under_game_score_y, 1000)
    cmd = 'adb shell input swipe {} {} {} {} {}'.format(x, y, x+60, y+60, press_time)
    print(cmd)
    os.system(cmd)
